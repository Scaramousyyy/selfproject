...
export function renderSlide(num, data) {
    ...
    switch(num) {
        case 1:
            content = glassWrapper(`
                ...
                <button onclick="next()" class="btn-primary mt-12 px-10 py-3 rounded-full font-bold shadow-md reveal-text delay-3">Next →</button>
            `);
            break;
        ...
        case 2:
            content = glassWrapper(`
                ...
                <p class="mt-4 text-gray-700 reveal-text delay-3">and the top #1 keyword in our conversation is:</p>
        ...
        case 3:
            content = glassWrapper(`
                ...
                <p class="text-lg text-gray-500 italic reveal-text delay-2">cafes just to hangout #ripmoney</p>
                <button onclick="next()" class="mt-12 text-xs uppercase tracking-widest text-gray-400 reveal-text delay-3">Next Stats</button>
            `);
            break;
        ...
        case 4:
            content = glassWrapper(`
                ...
                <p class="text-xl font-mono text-gray-400 italic reveal-text delay-3">times. crazy,,,</p>
                <button onclick="next()" class="mt-12 text-xs uppercase tracking-widest text-gray-400 reveal-text delay-4">Unbelievable →</button>
            `);
            break;

        case 5:
            content = glassWrapper(`
                <p class="text-md leading-relaxed text-gray-700 reveal-text delay-1 px-4">We also always do karaoke in ur car, Happy Puppy (kalau ada duit), or ur home.</p>
                <p class="mt-6 text-gray-700 reveal-text delay-2">idk how can u still survive after hearing my voice for</p>
                ...
                <button onclick="next()" class="mt-10 text-xs uppercase tracking-widest text-gray-400 reveal-text delay-4">Keep Singing!</button>
            `);
            break;

        case 6:
            content = glassWrapper(`
                <p class="text-xl text-gray-700 reveal-text delay-1">Lastly, we've been making</p>
                <h2 id="tiktok-anim" class="text-5xl font-black my-4 text-pink-500 reveal-text delay-2">0</h2>
                <p class="text-xl text-gray-700 reveal-text delay-2 px-4">drafts of TikTok's dance video, and there's more soon.... <br><span class="text-gray-400 text-sm">#jujurcapek</span></p>
                <button onclick="next()" class="mt-12 text-xs uppercase tracking-widest text-gray-400 reveal-text delay-3">HBD Soon</button>
            `);
            break;
        ...
        case 7:
            content = glassWrapper(`
                ...
                <p class="text-xl mb-6 italic text-gray-500 reveal-text delay-2">So, u are old, ig??</p>
                <p class="mt-8 text-pink-600 font-medium reveal-text delay-3 px-4">but thanks for letting me still exist in ur life for another year &lt;3</p>
                <button onclick="next()" class="mt-12 text-sm underline underline-offset-8 text-gray-400 uppercase tracking-tighter reveal-text delay-4">View Bill</button>
            `);
            break;
        ...
        case 8: // Music Receipt
            content = `
                <div class="flex flex-col items-center justify-center p-6 min-h-screen">
                    <p class="mb-4 text-sm text-gray-600 italic font-mono text-center reveal-text delay-1">anw, here's the receipt for using my bff service:</p>
                    <div class="receipt-container text-[10px] shadow-2xl reveal-text delay-2">
                        <h3 class="text-center font-black text-lg border-b-2 border-dashed border-black pb-2 mb-2 uppercase">Bff Service Bill</h3>
                    ...
                    <button onclick="next()" class="btn-primary mt-6 px-8 py-2 rounded-full font-bold reveal-text delay-4">NEXT</button>
                </div>`;
            break;
        ...
        case 9: // Voice Note
            content = glassWrapper(`
                ...
                <button onclick="next()" class="mt-12 text-pink-500 font-black animate-pulse tracking-widest reveal-text delay-4">LAST STEP →</button>
            `);
            break;
    ...
    setTimeout(() => {
        ...
        if (num === 6) animateValue("tiktok-anim", 0, data.stats.tiktokDrafts, 1500);
